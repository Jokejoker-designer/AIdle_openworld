# -*- coding: utf-8 -*-
"""BUILDINGS_FIDELITY_V14 â€” mockup match push (lighting + crisp topology).
...
EVID = Path(r"E:\AIdle_openworld\orchestration\evidence\town_grid_import_001\camera_match_v14")
RECEIPT = Path(r"E:\AIdle_openworld\orchestration\receipts\town_grid_import_001\BUILDINGS_FIDELITY_V14.json")
...
# Post-V14 residual signatures (must differ from V13)
V14_POST_SIG = {
    "cozy_market_stall_A": "market_v14_soft_scallop_awning_fabric_partial",
    "cozy_gazebo_A": "gazebo_v14_scale_overlap_depth_rail_partial",
    "cozy_well_house_A": "well_v14_low_brick_ring_tall_aframe_partial",
    "cozy_windmill_A": "windmill_v14_arch_window_ring_sail_length_partial",
    "cozy_bridge_arch_A": "bridge_v14_lavender_grey_hue_dense_pile_partial",
    "cozy_watchtower_A": "watchtower_v14_cabin_cutout_glow_windows_partial",
}

STRATEGIES = {
    "cozy_market_stall_A": "soft_scallop_fabric_awning_vs_tube_stripes",
    "cozy_gazebo_A": "scale_overlap_depth_plus_rail_ring_vs_flat_fishscale",
    "cozy_well_house_A": "lower_brick_body_taller_aframe_ratio_vs_prior",
    "cozy_windmill_A": "arch_window_ring_plus_longer_sails_vs_count_only",
    "cozy_bridge_arch_A": "lavender_grey_boost_hue_plus_packed_cobbles",
    "cozy_watchtower_A": "cabin_open_cutout_win_emit_vs_weak_frames",
}
...
LEVER = "residual_v14_priority_lookout_garden_bridge"
...
def build_market():
    ...
    # soft scallop fabric awning (mockup continuous curved canopy)
    for i in range(5):
        t = (i + 0.5) / 5.0
        x = -0.98 + t * 1.96
        m = Mpink if i % 2 == 0 else Mcream
        # thick soft slab with heavy bevel = fabric stripe
        o = cube(f"Awn{i}", (x, 0.10, 1.70), (0.42, 1.50, 0.20), m, 0.12)
        apply_rot(o, (0.35, 0, 0))
        # front scallop half-sphere hanging
        sph(f"Scallop{i}", (x, 0.82, 1.40), 0.20, m, (1.15, 0.85, 0.75), segs=10)
    # continuous front valance roll
    cube("Valance", (0, 0.88, 1.38), (2.15, 0.22, 0.18), Mpink, 0.10)
...
def build_gazebo():
    ...
    # solid green dome core for depth under scales (soft-clay volume)
    sph("DomeCore", (0, 0, 1.15), 1.05, Mg2, (1.05, 1.05, 0.55), segs=18)
...
def build_well():
    ...
    cyl("WellBody", (0, 0, 0.28), 0.62, 0.48, Ms, 16)
    ...
    cyl("PostL", (-0.78, 0, 1.25), 0.10, 2.15, Mw, 10)
    cyl("PostR", (0.78, 0, 1.25), 0.10, 2.15, Mw, 10)
    cube("Cross", (0, 0, 2.15), (1.65, 0.15, 0.15), Mw, 0.04)
...
def build_watchtower():
    ...
    Mce = mat("cabin_emit", (1.0, 0.86, 0.45), 0.30, emit=6.0)
    ...
    # open cabin cutouts with strong interior glow (mockup bld_08)
    # inset dark rims + deep win_emit planes so glow reads under town light
    cube("WinF_rim", (0, 0.46, 2.52), (0.52, 0.08, 0.46), Mframe, 0.02)
    cube("WinF_hole", (0, 0.42, 2.52), (0.40, 0.10, 0.34), Mce, 0.01)
    cube("WinF_deep", (0, 0.30, 2.52), (0.36, 0.20, 0.30), Mce, 0.01)
    cube("WinR_rim", (0.46, 0, 2.52), (0.08, 0.48, 0.42), Mframe, 0.02)
    cube("WinR_hole", (0.42, 0, 2.52), (0.10, 0.36, 0.30), Mce, 0.01)
    cube("WinR_deep", (0.30, 0, 2.52), (0.20, 0.32, 0.26), Mce, 0.01)
    sph("CabinGlow", (0, 0, 2.52), 0.38, Mce, (1.05, 1.05, 0.70), segs=10)
...
def export_with_camera_match(module_id: str) -> dict:
    ...
    return {
        ...
        "source": JOB,
        "visual": f"mockup_match_v14_{module_id}",
...
def update_catalog(rows: list[dict]) -> None:
    ...
    data["BUILDINGS_FIDELITY_V14"] = JOB
    CAT.write_text(json.dumps(data, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")


def main():
    log("start BUILDINGS_FIDELITY_V14 lighting+crisp topology n=6")
    ensure_addons()
    rows = []
    for mid in MODULES:
    ...
    for mid in MODULES:
        ...
        objects.append({
            "plot_id": PLOT[mid],
            "object_id": mid,
            "v14_strategy": STRATEGIES[mid],
            "lever": LEVER,
            "prior_V13_SIG": prior,
            "post_v14_signature": post,
    ...
    report = {
        ...
        "receipt_id": "BUILDINGS_FIDELITY_V14",
        "job": JOB,
