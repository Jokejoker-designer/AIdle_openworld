# -*- coding: utf-8 -*-
"""BUILDINGS_FIDELITY_V4 — HUMAN_AUTHORIZED_ONE_MORE_REMESH for 6 NEED_HUMAN buildings.

Signature-targeted only (not generic re-pass):
  MARKET  fruit_still_underdetailed
  GARDEN  fishscale_density
  WELL    overall form
  WINDMILL blade/tier detail
  BRIDGE  cobble arch soft-pile
  LOOKOUT roof detail

Does NOT touch HOME.BLD / workshop / barn / positions.
If still under 100% → NEED_HUMAN again (no auto V5).
...
def build_market():
    """V4 TARGET: fruit_still_underdetailed — dense produce piles, stems, multi-crate."""
    ...
    # V4: denser fruit — stacked piles with stems, front-readable
    crates = [
        (-0.60, Mred, 10, 0.11),
        (-0.05, Myel, 9, 0.10),
        (0.50, Mor, 11, 0.11),
    ]
    for i, (x, fruit_m, nfr, rad) in enumerate(crates):
        cube(f"Crate{i}", (x, 0.18, 0.98), (0.48, 0.48, 0.26), Mw2, 0.03)
        # lip
        cube(f"CrateLip{i}", (x, 0.18, 1.12), (0.50, 0.50, 0.04), Mw, 0.01)
        for j in range(nfr):
            layer = j // 4
            k = j % 4
            fx = x + ((k % 2) - 0.5) * 0.18 + (layer * 0.03)
            fy = 0.12 + ((k // 2) - 0.5) * 0.18
            fz = 1.20 + layer * 0.12
            sph(f"Fruit{i}_{j}", (fx, fy, fz), rad, fruit_m, segs=10)
            if j % 3 == 0 and fruit_m is Mred:
                cyl(f"Stem{i}_{j}", (fx, fy, fz + rad * 0.7), 0.015, 0.05, Mstem, 5)
    # side grapes / greens basket
    cube("BasketG", (-0.95, 0.35, 0.95), (0.28, 0.28, 0.18), Mw, 0.02)
    for j in range(6):
        sph(f"Grape{j}", (-0.95 + (j % 3 - 1) * 0.07, 0.35 + (j // 3 - 0.5) * 0.08, 1.10), 0.06, Mgreen, segs=8)
    cube("RegBase", (0.85, 0.45, 0.98), (0.30, 0.24, 0.14), Mreg, 0.03)
    cube("RegTop", (0.85, 0.45, 1.12), (0.24, 0.18, 0.16), Mreg, 0.02)
    sph("RegKnob", (0.92, 0.52, 1.08), 0.035, Mgold)
...
def build_bridge():
    """V4 TARGET: soft_pile → sharp masonry arch — fewer larger blocks, clear keystone, open void."""
    ...
    mats = [Ms1, Ms2, Ms3]
    # Single-layer arch ring of LARGER blocks (not multi-offset soft pile)
    n_arch = 9
    for i in range(n_arch):
        t = i / (n_arch - 1)
        ang = math.pi * t
        x = 1.10 * math.cos(ang)
        z = 0.10 + 0.85 * math.sin(ang)
        # elongated cobble along tangent
        o = sph(f"Arch{i}", (x, 0, z), 0.26, mats[i % 3], (1.15, 0.95, 0.95), segs=10)
        ...
        try:
            ...
            pass
        o.select_set(False)
    # keystone emphasize apex
    sph("Keystone", (0, 0, 0.95), 0.30, Ms3, (1.2, 1.0, 1.0), segs=10)
    # abutments (two solid feet) — architectural, not rubble
    for side, x in (("L", -1.15), ("R", 1.15)):
        cube(f"Abut{side}", (x, 0, 0.28), (0.45, 0.70, 0.55), Ms2, 0.06)
        cube(f"AbutTop{side}", (x, 0, 0.58), (0.40, 0.65, 0.18), Ms1, 0.04)
    # flat deck of 5 large stones only
    for i in range(5):
        t = (i + 0.5) / 5.0
        x = -0.85 + t * 1.7
        sph(f"Deck{i}", (x, 0, 0.88), 0.24, mats[i % 3], (1.25, 1.05, 0.45), segs=10)
    # low rail: 3 stones per side (not dense pile)
    for side, y in (("L", 0.38), ("R", -0.38)):
        for i, t in enumerate([0.15, 0.5, 0.85]):
            x = -0.9 + t * 1.8
            z = 0.70 + 0.18 * math.sin(t * math.pi)
            sph(f"Rail{side}{i}", (x, y, z), 0.16, mats[(i + 1) % 3], (1.0, 0.75, 0.7), segs=8)
    for i, (x, y) in enumerate([(-1.25, 0.55), (1.25, -0.5), (0.0, 0.65)]):
...
def build_watchtower():
    """V4 TARGET: roof_detail_partial — denser conical thatch scales + eave ring + peak finial."""
    ...
    # V4 denser conical roof (SSOT brown petal thatch)
    rings = [
        (0.08, 3.20, 1, 0.16),
        (0.22, 3.08, 8, 0.13),
        (0.38, 2.96, 12, 0.14),
        ...
        (0.66, 2.72, 16, 0.15),
        (0.78, 2.58, 18, 0.15),
        (0.90, 2.48, 16, 0.14),  # eave flare
    ]
    mats = [Mr1, Mr2, Mr3]
    for ring, (rad, z, n, sc) in enumerate(rings):
        if n == 1:
            sph("RoofCap", (0, 0, z), sc, Mr3, (1.1, 1.1, 0.75))
            # finial
            cyl("Finial", (0, 0, z + 0.18), 0.04, 0.20, Mw, 6)
            continue
        for i in range(n):
            ang = TAU * i / n + (0.1 if ring % 2 else 0.0)
            x, y = rad * math.cos(ang), rad * math.sin(ang)
            sph(f"Sh{ring}_{i}", (x, y, z), sc, mats[(i + ring) % 3], (1.25, 1.0, 0.48), segs=8)
    # eave wood ring under roof
    cyl("EaveRing", (0, 0, 2.42), 0.88, 0.08, Mband, 16)
    ...
    sph("Dust1", (0.15, 0.1, 3.40), 0.06, mat("M_dust", (0.85, 0.72, 0.55), 0.8))
    parent_all(r)
    return r
...
def export_module(module_id: str) -> dict:
    ...
    return {"module_id": module_id, "glb_sha256": dig, "bytes": size, "source": JOB, "visual": f"mockup_{module_id}_v4", "mockup_ssot": module_id}
...
def update_catalog(rows: list[dict]) -> None:
    ...
    data["buildings_fidelity_v4"] = JOB
    CAT.write_text(json.dumps(data, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    log(f"catalog modules={len(data['modules'])}")


def main():
    log("start BUILDINGS_FIDELITY_V4 HUMAN_AUTHORIZED_ONE_MORE_REMESH n=6")
    rows = []
    for mid in MODULES:
        rows.append(export_module(mid))
    update_catalog(rows)
    report = QUAR / "BUILDINGS_FIDELITY_V4_report.json"
