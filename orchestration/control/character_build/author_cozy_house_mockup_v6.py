...
COL = {
    "wall": (0.99, 0.94, 0.88),
    "roof_a": (0.99, 0.76, 0.40),
    "roof_b": (1.00, 0.88, 0.42),
    "roof_c": (0.98, 0.80, 0.52),
...
def build():
    ...
    cube("Front", (0, 0.48, 0.52), (0.90, 0.16, 0.80), M["wall"], 0.10)
    ...
    for sign, nm in ((1, "GableF"), (-1, "GableB")):
        ...
        o = bpy.context.active_object
        o.name = nm
        o.scale = (1.50, 0.92, 0.18)
        o.rotation_euler = Euler((sign * math.radians(32), 0, 0), "XYZ")
        bpy.ops.object.transform_apply(location=False, rotation=True, scale=True)
        fin(o, M["ridge"], 0.04)

    # Roof ends (gable triangles approximated by cubes)
    cube("GableEndL", (-0.68, 0, 1.45), (0.12, 0.95, 0.55), M["wall"], 0.06)
    cube("GableEndR", (0.68, 0, 1.45), (0.12, 0.95, 0.55), M["wall"], 0.06)
    ...
    for row in range(7):
        ...
        for col in range(n_col):
            x = (col - (n_col - 1) / 2) * 0.165
            if row % 2:
                x += 0.08
            if abs(x) > 0.78:
                continue
            m = mats_r[(row + col) % 3]
            o = sph(f"Tile{row}_{col}", (x, y, z), 0.108, m, (1.35, 1.15, 0.40))
    ...
    cube("DoorWall", (0, 0.50, 0.50), (0.44, 0.12, 0.78), M["wall"], 0.05)
...
def main():
    ...
    preview = RENDER_DIR / f"{MODULE_ID}_blender_preview_v6.png"
    try:
