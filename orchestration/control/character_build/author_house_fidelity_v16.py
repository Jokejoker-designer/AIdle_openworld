# -*- coding: utf-8 -*-
"""HOUSE_FIDELITY_V16 â€” dense fish-scale on prism roof (no object rotation).
...
def roof_prism(name, m):
    """Triangular prism roof â€” verts only, no object rotation (Godot-safe)."""
...
def fish_scale(name, u, t, front, m):
    ...
    Front slope: eave (y=+0.78, z=1.10) â†’ peak (y=0, z=1.95)
    Back slope:  eave (y=-0.78, z=1.10) â†’ peak (y=0, z=1.95)
    u in [-1,1] along X, t in [0,1] eaveâ†’peak
    """
    if front:
        y = 0.78 * (1 - t) + 0.0 * t
    else:
    ...
    x = u * 0.82
    # Flattened sphere = scallop â€œcoinâ€
...
def build():
    ...
    cube("Door", (0, 0.64, 0.45), (0.38, 0.06, 0.68), Md, 0.05)
