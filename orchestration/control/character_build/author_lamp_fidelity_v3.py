...
Memit = mat("M_emit", (1.0, 0.85, 0.35), 0.3, emit=6.5)
Mleaf = mat("M_leaf", (0.32, 0.78, 0.30), 0.55)
Mvine = mat("M_vine", (0.42, 0.68, 0.45), 0.6)
Mc = mat("M_cyan", (0.25, 0.90, 1.0), 0.35, emit=2.2)
...
# denser ivy (SSOT cyber lamp has climbing vines + leaves)
for i, z in enumerate([0.32, 0.42, 0.52, 0.62, 0.72, 0.82, 0.92, 1.02, 1.12, 1.22]):
    side = 1 if i % 2 == 0 else -1
    sph(f"Leaf{i}", (0.12 + 0.015 * (i % 3), 0.10 * side, z), 0.055, Mleaf, (1.4, 0.75, 0.55))
    cyl(f"Vine{i}", (0.10, 0.08 * side, z - 0.03), 0.014, 0.12, Mvine)
    if i % 3 == 0:
        sph(f"LeafB{i}", (-0.10, 0.08 * side, z + 0.05), 0.04, Mleaf, (1.2, 0.6, 0.45))
# stronger cyan circuit + lantern rim glow
cube("Cyan3", (0.09, 0.0, 1.00), (0.03, 0.03, 0.35), Mc)
cube("Cyan4", (-0.09, 0.0, 0.60), (0.03, 0.03, 0.30), Mc)
sph("Glow2", (0.50, 0, 1.10), 0.14, Memit, (1.1, 1.1, 1.1))
cube("LanternRim", (0.50, 0, 1.28), (0.28, 0.28, 0.04), Mc)
...
for m in data.get("modules", []):
    if m.get("module_id") == MODULE:
        ...
        m["source"] = JOB
        m["visual"] = "mockup_lamp_v3"
CAT.write_text(json.dumps(data, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
print(f"LAMP_V3 {GAME.stat().st_size} {dig[:16]}")
