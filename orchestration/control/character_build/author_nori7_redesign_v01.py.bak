# -*- coding: utf-8 -*-
"""Nori-7 visual redesign V01 — offline Blender 5.2 authoring.
...
def author_clip(arm_obj, name: str, duration_s: float, keyframes_fn) -> dict:
    ...
    # Count fcurves
    fcount = len(act.fcurves) if act.fcurves else 0
    return {
        "action_name": name,
...
def main() -> int:
    ...
    # Store all actions on Nori7 NLA or sequential — glTF exports all actions if we push them
    # Keep last action on armature; push others to NLA strips so export keeps them
    arm.animation_data_create()
    track = arm.animation_data.nla_tracks.new()
    track.name = "nori7_clips"
    ...
    for clip_name, dur, fn in clips_spec():
        ...
        if act:
            ...
            start = 1
            try:
                track.strips.new(clip_name, start, act)
            except Exception:
                # strip name collision — push with unique
                track.strips.new(f"{clip_name}_strip", start, act)
