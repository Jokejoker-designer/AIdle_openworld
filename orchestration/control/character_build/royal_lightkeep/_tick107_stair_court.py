# -*- coding: utf-8 -*-
"""Tick #107: main stair + courtyard U-wrap polish (prompt priorities).
Continue PASS8_V84_CROWN_GABLES. Scale lock 24x19x38. No densify. No FINAL."""
import bpy
import bmesh
import os
import shutil
...
WORK = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V84_CROWN_GABLES.blend")
...
# ========== Unhide key U-wrap if hidden ==========
for name in ("UWRAP_REAR_BAR", "UWRAP_WING_L", "UWRAP_WING_R", "UWRAP_COURT_PAVE",
             "CHANNEL_WALL_L", "CHANNEL_WALL_R", "CHANNEL_FLOOR", "CHANNEL_THRESH"):
    o = bpy.data.objects.get(name)
    if o:
        o.hide_render = False
        o.hide_viewport = False
        n += 1
print("UWRAP_UNHIDE", n)

# Reinforce U-wrap dimensions
for name, size, z, xy in (
    ("UWRAP_REAR_BAR", (13.5, 1.6, 6.0), 0.2, (CX, -4.3)),
    ("UWRAP_WING_L", (2.0, 10.0, 5.8), 0.2, (CX - 7.0, 1.0)),
    ("UWRAP_WING_R", (2.0, 10.0, 5.8), 0.2, (CX + 7.0, 1.0)),
):
    o = ensure_cube(name)
...
set_size(o, 9.0, 6.5, 0.12, bottom_z=0.08, center_xy=(CX, 0.8))
assign(o, MAT_PATH)
apply_scale(o)

# ========== Monumental stair cascade (refresh 6 treads + landing + cheeks) ==========
o = ensure_cube("STAIR_LANDING_MAIN")
set_size(o, 5.4, 2.6, 0.4, bottom_z=0.12, center_xy=(CX, 5.7))
assign(o, MAT_STONE)
apply_scale(o)
soft_bevel(o, 0.04)
...
for i in range(7):
    ...
    set_size(o, w, 0.55, 0.3, bottom_z=z, center_xy=(CX, y))
    assign(o, MAT_STONE if i % 2 == 0 else MAT_PATH)
    apply_scale(o)
    soft_bevel(o, 0.03)

for side, sx in (("L", -2.75), ("R", 2.75)):
    o = ensure_cube(f"STAIR_CHEEK_{side}")
    set_size(o, 0.5, 4.4, 1.8, bottom_z=0.15, center_xy=(CX + sx, 7.0))
    assign(o, MAT_DARK)
    apply_scale(o)
    soft_bevel(o, 0.05)
    cap = ensure_cube(f"STAIR_CHEEK_CAP_{side}")
    set_size(cap, 0.58, 4.4, 0.14, bottom_z=1.9, center_xy=(CX + sx, 7.0))
    assign(cap, MAT_GOLD)
    apply_scale(cap)

# Rail posts (fewer, cleaner)
for i in range(5):
    y = 8.2 - i * 0.6
    z = 0.4 + i * 0.28
    for side, sx in (("L", -2.5), ("R", 2.5)):
        p = ensure_cube(f"STAIR_RAIL_POST_{side}_{i}")
        set_size(p, 0.16, 0.16, 0.9, bottom_z=z, center_xy=(CX + sx, y))
        assign(p, MAT_STONE)
        apply_scale(p)
for side, sx in (("L", -2.5), ("R", 2.5)):
    r = ensure_cube(f"STAIR_RAIL_TOP_{side}")
    set_size(r, 0.12, 3.2, 0.12, bottom_z=2.1, center_xy=(CX + sx, 6.8))
    assign(r, MAT_GOLD)
    apply_scale(r)

# Channel approach reinforce
set_size(ensure_cube("CHANNEL_FLOOR"), 4.2, 4.8, 0.12, bottom_z=0.1, center_xy=(CX, 7.2))
assign(bpy.data.objects["CHANNEL_FLOOR"], MAT_PATH)
apply_scale(bpy.data.objects["CHANNEL_FLOOR"])
for side, sx in (("L", -2.3), ("R", 2.3)):
    w = ensure_cube(f"CHANNEL_WALL_{side}")
    set_size(w, 0.5, 4.2, 3.0, bottom_z=0.15, center_xy=(CX + sx, 7.2))
    assign(w, MAT_STONE)
    apply_scale(w)

print("STAIR_COURT_DONE")
...
**Updated:** 2026-07-27 (job `019f9eec8b6f` tick **#107**, 5m)  
...
| **accepted (user)** | **false** |
| **Current** | **PASS8 V85 STAIR_COURT** |
| **Scale** | FP **{bw:.1f}×{bd:.1f}** (≤24×19) · top Z **{maxz:.1f}** |
| **Scheduler** | `019f9eec8b6f` every **5 min** |
| **Pipeline** | IN_PROGRESS (stair + U-wrap polish) |

---

## Tick #107 — executed (P0/P1 prompt priorities)

### Edits
1. **U-wrap** unhide + reinforce rear bar / wings / court pave
2. **Main stair** cascade refresh (7 treads + landing + cheeks/gold + rails)
3. **Channel** approach walls/floor

### Files
| Role | Path |
|------|------|
| Work | `ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V85_STAIR_COURT.blend` |
...
| Scale | **9.0** |
| Stair / approach | ~8.5 |
| Courtyard U-wrap | ~8.45 |
| Overall | **~8.55** |

### Verdict
Not FINAL. Prompt gaps (stair/U-wrap) reinforced. D1 modular core remains. Human overlay for FINAL.
"""

loop_state = f"""# BUILD LOOP STATE

**Updated:** 2026-07-27 · tick **#107** · job `019f9eec8b6f` (5m)  
**ASSET_FINAL_COMPLETE:** false · **Current:** PASS8_V85_STAIR_COURT  

## Tick #107 (P0/P1)
- Stair cascade + U-wrap + channel
- Bounds {bw:.1f}×{bd:.1f}×{bh:.1f}
- Backup: BACKUP_LOOP_{STAMP}
- Saved: PASS8_V85_STAIR_COURT → PASS1D / FINAL

## Next
Human overlay preferred for FINAL; avoid cube densify
"""

dev = f"""# INTERMEDIATE DEVIATION REPORT — tick #107

**Asset:** ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01
**Stamp:** {STAMP}
**File:** PASS8_V85_STAIR_COURT / PASS1D
**ASSET_FINAL_COMPLETE:** false
**accepted (user):** false
...
## Matches
- Monumental stair + U-wrap courtyard + channel
- Crown gables + curved arches/lancets + hip roof retained
- Scale lock held
...
| D2 | P1 | Roof/crown still simplified vs mockup |
...
out_blend = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V85_STAIR_COURT.blend")
