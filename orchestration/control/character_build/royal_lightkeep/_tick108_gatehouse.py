# -*- coding: utf-8 -*-
"""Tick #108: gatehouse / portal front silhouette polish.
Continue PASS8_V85_STAIR_COURT. Scale lock 24x19x38. No densify. No FINAL."""
import bpy
import bmesh
import os
import shutil
...
WORK = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V85_STAIR_COURT.blend")
...
# Outer gatehouse mass (front protrusion)
gh = ensure_cube("GATEHOUSE_MASS")
set_size(gh, 7.5, 3.2, 9.5, bottom_z=0.2, center_xy=(CX, 7.8))
assign(gh, MAT_STONE)
apply_scale(gh)
soft_bevel(gh, 0.08)
n += 1

# Gate opening boolean through gatehouse
cut = ensure_cube("GATEHOUSE_OPEN_CUT")
set_size(cut, 3.8, 4.0, 6.0, bottom_z=0.3, center_xy=(CX, 8.0))
apply_scale(cut)
if add_bool_diff(gh, cut, "GATEHOUSE_OPEN_BOOL"):
    print("BOOL GATEHOUSE")
    n += 1

# Flanking turrets on gatehouse
for side, sx in (("L", -3.6), ("R", 3.6)):
    t = ensure_cube(f"GATEHOUSE_TURRET_{side}")
    set_size(t, 1.8, 1.8, 11.5, bottom_z=0.2, center_xy=(CX + sx, 8.2))
    assign(t, MAT_STONE)
    apply_scale(t)
    soft_bevel(t, 0.06)
    cap = ensure_cube(f"GATEHOUSE_TURRET_CAP_{side}")
    set_size(cap, 2.1, 2.1, 0.35, bottom_z=11.5, center_xy=(CX + sx, 8.2))
    assign(cap, MAT_DARK)
    apply_scale(cap)
    roof = ensure_cube(f"GATEHOUSE_TURRET_ROOF_{side}")
    set_size(roof, 1.5, 1.5, 1.8, bottom_z=11.8, center_xy=(CX + sx, 8.2))
    assign(roof, MAT_ROOF)
    apply_scale(roof)
    tip = ensure_cube(f"GATEHOUSE_TURRET_TIP_{side}")
    set_size(tip, 0.25, 0.25, 0.7, bottom_z=13.4, center_xy=(CX + sx, 8.2))
    assign(tip, MAT_GOLD)
    apply_scale(tip)
    n += 4

# Battlement merlons on gatehouse top (few)
for i, xoff in enumerate([-2.5, -1.0, 1.0, 2.5]):
    m = ensure_cube(f"GATEHOUSE_MERLON_{i}")
    set_size(m, 0.7, 0.8, 1.0, bottom_z=9.5, center_xy=(CX + xoff, 8.5))
    assign(m, MAT_STONE)
    apply_scale(m)
    n += 1

# Gold cornice on gatehouse
corn = ensure_cube("GATEHOUSE_CORNICE")
set_size(corn, 7.8, 3.4, 0.25, bottom_z=9.3, center_xy=(CX, 7.8))
assign(corn, MAT_GOLD)
apply_scale(corn)
n += 1

# Ensure curved arch and portal elements visible and forward
for name in ("PORTAL_CURVED_ARCH", "PORTAL_CURVED_ARCH_INNER", "PORTAL_CURVED_FINIAL",
             "PORTAL_GATE_L", "PORTAL_GATE_R", "PORTAL_COLONNETTE_L", "PORTAL_COLONNETTE_R"):
    o = bpy.data.objects.get(name)
    if o:
        o.hide_render = False
        o.hide_viewport = False

# Push curved arch slightly forward for gatehouse depth
arch = bpy.data.objects.get("PORTAL_CURVED_ARCH")
if arch:
    arch.location.y = max(arch.location.y, 7.3)
    n += 1

print("GATEHOUSE_PARTS", n)
...
**Updated:** 2026-07-27 (job `019f9eec8b6f` tick **#108**, 5m)  
...
| **accepted (user)** | **false** |
| **Current** | **PASS8 V86 GATEHOUSE** |
| **Scale** | FP **{bw:.1f}×{bd:.1f}** (≤24×19) · top Z **{maxz:.1f}** |
| **Scheduler** | `019f9eec8b6f` every **5 min** |
| **Pipeline** | IN_PROGRESS (gatehouse front silhouette) |

---

## Tick #108 — executed (P1 front)

### Edits
1. **Gatehouse mass** + opening boolean
2. **Flanking turrets** (caps/roofs/gold tips)
3. **Battlement merlons** (4) + gold cornice; curved arch forward

### Files
| Role | Path |
|------|------|
| Work | `ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V86_GATEHOUSE.blend` |
...
|------|-------|
| Scale | **9.0** |
| Front / gatehouse | ~8.55 |
| Overall | **~8.55** |

### Verdict
Not FINAL. Front gatehouse reading stronger. D1 modular core remains. Human overlay for FINAL.
"""

loop_state = f"""# BUILD LOOP STATE

**Updated:** 2026-07-27 · tick **#108** · job `019f9eec8b6f` (5m)  
**ASSET_FINAL_COMPLETE:** false · **Current:** PASS8_V86_GATEHOUSE  

## Tick #108 (P1)
- Gatehouse mass + turrets + merlons + cornice
- Bounds {bw:.1f}×{bd:.1f}×{bh:.1f}
- Backup: BACKUP_LOOP_{STAMP}
- Saved: PASS8_V86_GATEHOUSE → PASS1D / FINAL

## Next
Human overlay preferred; avoid densify
"""

dev = f"""# INTERMEDIATE DEVIATION REPORT — tick #108

**Asset:** ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01
**Stamp:** {STAMP}
**File:** PASS8_V86_GATEHOUSE / PASS1D
**ASSET_FINAL_COMPLETE:** false
**accepted (user):** false
...
## Matches
- Explicit gatehouse with opening + flank turrets
- Stair/U-wrap/crown/lancets/hip roof retained
- Scale lock held
...
out_blend = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V86_GATEHOUSE.blend")
