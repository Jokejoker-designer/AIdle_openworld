# -*- coding: utf-8 -*-
"""Tick #111: sparse quality side buttresses + inventory.
Continue PASS8_V88_CURTAIN_MERLON2. Scale lock 24x19x38. No densify. No FINAL."""
import bpy
import bmesh
import os
import shutil
...
WORK = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V88_CURTAIN_MERLON2.blend")
...
# Unhide existing FLY_* if present, else create 3 per side sparse
for side, sx in (("L", -1.0), ("R", 1.0)):
    for i, (y, h) in enumerate([(-2.5, 12.0), (1.0, 14.0), (4.5, 11.0)]):
        x_outer = CX + sx * 9.2
        x_inner = CX + sx * 5.8
        pier = ensure_cube(f"QBUTT_PIER_{side}_{i}")
        set_size(pier, 1.2, 1.2, h, bottom_z=0.15, center_xy=(x_outer, CY + y))
        assign(pier, MAT_STONE)
        apply_scale(pier)
        soft_bevel(pier, 0.06)
        # stepped setback on pier
        step = ensure_cube(f"QBUTT_STEP_{side}_{i}")
        set_size(step, 1.5, 1.5, 1.2, bottom_z=0.15, center_xy=(x_outer, CY + y))
        assign(step, MAT_DARK)
        apply_scale(step)
        # arm
        arm = ensure_cube(f"QBUTT_ARM_{side}_{i}")
        set_size(arm, abs(x_outer - x_inner) + 0.4, 0.6, 0.7, bottom_z=h - 2.2, center_xy=((x_outer + x_inner) / 2, CY + y))
        arm.rotation_euler = Euler((0, math.radians(16 if side == "R" else -16), 0), "XYZ")
        assign(arm, MAT_STONE)
        apply_scale(arm)
        # pinnacle
        pin = ensure_cube(f"QBUTT_PIN_{side}_{i}")
        set_size(pin, 0.5, 0.5, 2.0, bottom_z=h, center_xy=(x_outer, CY + y))
        assign(pin, MAT_STONE)
        apply_scale(pin)
        tip = ensure_cube(f"QBUTT_TIP_{side}_{i}")
        set_size(tip, 0.22, 0.22, 0.7, bottom_z=h + 1.9, center_xy=(x_outer, CY + y))
        assign(tip, MAT_GOLD)
        apply_scale(tip)
        n += 5
...
# Hide old dense FLY_* if still visible (replace with QBUTT)
hidden = 0
for o in list(bpy.data.objects):
    if o.name.startswith("FLY_"):
        o.hide_render = True
        o.hide_viewport = True
        hidden += 1
print("HIDDEN_OLD_FLY", hidden)
...
**Updated:** 2026-07-27 (job `019f9eec8b6f` tick **#111**, 5m)  
...
| **accepted (user)** | **false** |
| **Current** | **PASS8 V89 BUTTRESS** |
| **Scale** | FP **{bw:.1f}×{bd:.1f}** (≤24×19) · top Z **{maxz:.1f}** |
| **Scheduler** | `019f9eec8b6f` every **5 min** |
| **Pipeline** | IN_PROGRESS (sparse quality buttresses) |

---

## Tick #111 — executed (P1 side silhouette)

### Edits
1. **Sparse buttresses** — 3 per side (pier/step/arm/pin/gold tip)
2. Hide old dense FLY_* ({hidden})
3. Inventory
...
| Role | Path |
|------|------|
| Work | `ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V89_BUTTRESS.blend` |
...
|------|-------|
| Scale | **9.0** |
| Side silhouette | ~8.55 |
| Overall | **~8.55** |

### Verdict
Not FINAL. Side buttress language cleaned. D1 modular core remains. Human overlay for FINAL.
"""

loop_state = f"""# BUILD LOOP STATE

**Updated:** 2026-07-27 · tick **#111** · job `019f9eec8b6f` (5m)  
**ASSET_FINAL_COMPLETE:** false · **Current:** PASS8_V89_BUTTRESS  

## Tick #111 (P1)
- Sparse QBUTT 3×L/R; hide old FLY
- Bounds {bw:.1f}×{bd:.1f}×{bh:.1f} · meshes {mesh_n}
- Backup: BACKUP_LOOP_{STAMP}
- Saved: PASS8_V89_BUTTRESS → PASS1D / FINAL

## Next
Human overlay preferred; plateau
"""

dev = f"""# INTERMEDIATE DEVIATION REPORT — tick #111

**Asset:** ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01
**Stamp:** {STAMP}
**File:** PASS8_V89_BUTTRESS / PASS1D
**ASSET_FINAL_COMPLETE:** false
**accepted (user):** false
...
## Matches
- Sparse side buttresses with arms/pinnacles
- Curtain merlons + gatehouse + stair + crown retained
- Scale lock held
...
out_blend = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V89_BUTTRESS.blend")
