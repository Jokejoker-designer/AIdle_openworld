# -*- coding: utf-8 -*-
"""Tick #57: side/rear buttress densify. Scale lock 24x19x38."""
import bpy
import os
import shutil
import bmesh
...
WORK = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V34_DORM2.blend")
...
# Left side buttresses along hall
for i, y in enumerate([-4.0, -1.0, 2.0, 5.0]):
    set_size(ensure_cube(f"BUTT2_L_{i}"), 1.0, 0.9, 9.5, bottom_z=0.2, center_xy=(-10.6, y))
    assign(bpy.data.objects[f"BUTT2_L_{i}"], MAT_STONE)
    set_size(ensure_cube(f"BUTT2_L_CAP_{i}"), 1.2, 1.1, 0.4, bottom_z=9.5, center_xy=(-10.6, y))
    assign(bpy.data.objects[f"BUTT2_L_CAP_{i}"], MAT_DARK)

# Right side buttresses
for i, y in enumerate([-3.5, 0.0, 3.5]):
    set_size(ensure_cube(f"BUTT2_R_{i}"), 1.0, 0.9, 9.0, bottom_z=0.2, center_xy=(10.8, y))
    assign(bpy.data.objects[f"BUTT2_R_{i}"], MAT_STONE)
    set_size(ensure_cube(f"BUTT2_R_CAP_{i}"), 1.2, 1.1, 0.4, bottom_z=9.0, center_xy=(10.8, y))
    assign(bpy.data.objects[f"BUTT2_R_CAP_{i}"], MAT_DARK)

# Rear buttresses
for i, x in enumerate([-8.0, -4.0, 0.0, 4.0, 8.0]):
    set_size(ensure_cube(f"BUTT2_REAR_{i}"), 0.9, 1.0, 8.5, bottom_z=0.2, center_xy=(x, -7.0))
    assign(bpy.data.objects[f"BUTT2_REAR_{i}"], MAT_STONE)

# Tower base flying-buttress stubs (angled as boxes)
for i, (x, y) in enumerate([(-4.5, 4.5), (4.5, 4.5), (-4.5, -1.5), (4.5, -1.5)]):
    set_size(ensure_cube(f"BUTT2_FLY_{i}"), 1.4, 1.4, 12.0, bottom_z=0.3, center_xy=(x, y))
    assign(bpy.data.objects[f"BUTT2_FLY_{i}"], MAT_STONE)

for o in list(bpy.data.objects):
    if o.type != "MESH" or o.hide_render:
        continue
    if o.name.startswith("BUTT2_"):
        try:
...
out_blend = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V35_BUTT2.blend")
