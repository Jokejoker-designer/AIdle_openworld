# -*- coding: utf-8 -*-
"""Tick #60: front gallery/loggia band under tower mid. Scale lock 24x19x38."""
import bpy
import os
import shutil
import bmesh
...
WORK = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V37_CLAMP2.blend")
...
# Gallery floor slab mid tower front
set_size(ensure_cube("GALLERY_FLOOR"), 8.5, 2.2, 0.35, bottom_z=9.5, center_xy=(CX, CY + 3.2))
assign(bpy.data.objects["GALLERY_FLOOR"], MAT_STONE)

# Gallery posts
for i, xoff in enumerate([-3.5, -1.5, 1.5, 3.5]):
    set_size(ensure_cube(f"GALLERY_POST_{i}"), 0.35, 0.35, 3.2, bottom_z=9.8, center_xy=(CX + xoff, CY + 3.8))
    assign(bpy.data.objects[f"GALLERY_POST_{i}"], MAT_STONE)

# Gallery roof canopy
set_size(ensure_cube("GALLERY_ROOF"), 8.8, 2.4, 0.4, bottom_z=12.8, center_xy=(CX, CY + 3.3))
assign(bpy.data.objects["GALLERY_ROOF"], MAT_DARK)
set_size(ensure_cube("GALLERY_GOLD"), 8.5, 0.2, 0.15, bottom_z=13.15, center_xy=(CX, CY + 4.0))
assign(bpy.data.objects["GALLERY_GOLD"], MAT_GOLD)

# Balustrade rail
set_size(ensure_cube("GALLERY_RAIL"), 8.0, 0.15, 0.25, bottom_z=11.2, center_xy=(CX, CY + 4.0))
assign(bpy.data.objects["GALLERY_RAIL"], MAT_WOOD)

# Dark openings between posts
for i, xoff in enumerate([-2.5, 0.0, 2.5]):
    set_size(ensure_cube(f"GALLERY_VOID_{i}"), 1.5, 0.3, 2.4, bottom_z=10.0, center_xy=(CX + xoff, CY + 3.9))
    assign(bpy.data.objects[f"GALLERY_VOID_{i}"], MAT_DARK)

for o in list(bpy.data.objects):
    if o.type != "MESH" or o.hide_render:
        continue
    if o.name.startswith("GALLERY_"):
        try:
...
out_blend = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V38_GALLERY.blend")
