# -*- coding: utf-8 -*-
"""Tick #68: window sills + lintels on tower front openings. Scale lock 24x19x38."""
import bpy
import os
import shutil
import bmesh
...
WORK = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V45_PAVE.blend")
...
# Sills + lintels matching tower mid window grid (front face)
n = 0
for row, z in enumerate([11.0, 15.5, 19.5, 25.5]):
    for col, xoff in enumerate([-1.5, 1.5]):
        # sill below window
        set_size(ensure_cube(f"SILL_F_{row}_{col}"), 1.4, 0.4, 0.18, bottom_z=z - 0.15, center_xy=(CX + xoff, CY + 2.9))
        assign(bpy.data.objects[f"SILL_F_{row}_{col}"], MAT_STONE)
        # lintel above
        set_size(ensure_cube(f"LINTEL_F_{row}_{col}"), 1.5, 0.35, 0.22, bottom_z=z + 1.55, center_xy=(CX + xoff, CY + 2.9))
        assign(bpy.data.objects[f"LINTEL_F_{row}_{col}"], MAT_STONE)
        n += 2

# Hall front window sills
for i, x in enumerate([-8.0, -5.5, -3.0]):
    set_size(ensure_cube(f"SILL_HALL_{i}"), 1.4, 0.35, 0.16, bottom_z=5.6, center_xy=(x, 6.45))
    assign(bpy.data.objects[f"SILL_HALL_{i}"], MAT_STONE)
    set_size(ensure_cube(f"LINTEL_HALL_{i}"), 1.5, 0.3, 0.2, bottom_z=7.8, center_xy=(x, 6.45))
    assign(bpy.data.objects[f"LINTEL_HALL_{i}"], MAT_GOLD)
    n += 2

print("SILLS_LINTELS", n)

for o in list(bpy.data.objects):
    if o.type != "MESH" or o.hide_render:
        continue
    if o.name.startswith(("SILL_", "LINTEL_")):
        try:
...
out_blend = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V46_SILLS.blend")
