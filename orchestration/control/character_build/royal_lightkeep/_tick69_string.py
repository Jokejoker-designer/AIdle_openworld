# -*- coding: utf-8 -*-
"""Tick #69: multi-level string courses around tower + hall. Scale lock 24x19x38."""
import bpy
import os
import shutil
import bmesh
...
WORK = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V46_SILLS.blend")
...
# Tower ring string courses at multiple heights
for i, z in enumerate([8.0, 14.0, 20.0, 26.0, 30.5]):
    s = 7.8 - i * 0.25
    set_size(ensure_cube(f"STRING_TOWER_{i}"), s, s, 0.28, bottom_z=z, center_xy=(CX, CY))
    assign(bpy.data.objects[f"STRING_TOWER_{i}"], MAT_STONE if i % 2 == 0 else MAT_GOLD)

# Hall front string
set_size(ensure_cube("STRING_HALL_MID"), 10.0, 0.35, 0.28, bottom_z=8.5, center_xy=(-6.5, 6.2))
assign(bpy.data.objects["STRING_HALL_MID"], MAT_STONE)
set_size(ensure_cube("STRING_HALL_TOP"), 10.0, 0.35, 0.25, bottom_z=11.2, center_xy=(-6.5, 6.2))
assign(bpy.data.objects["STRING_HALL_TOP"], MAT_GOLD)

# Wing string
set_size(ensure_cube("STRING_WING_MID"), 5.5, 0.35, 0.28, bottom_z=8.0, center_xy=(8.0, 5.4))
assign(bpy.data.objects["STRING_WING_MID"], MAT_STONE)

# Outer wall top string on WALL65
set_size(ensure_cube("STRING_WALL_FRONT"), 14.0, 0.4, 0.22, bottom_z=6.3, center_xy=(1.0, 8.7))
assign(bpy.data.objects["STRING_WALL_FRONT"], MAT_GOLD)

for o in list(bpy.data.objects):
    if o.type != "MESH" or o.hide_render:
        continue
    if o.name.startswith("STRING_"):
        try:
...
out_blend = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V47_STRING.blend")
