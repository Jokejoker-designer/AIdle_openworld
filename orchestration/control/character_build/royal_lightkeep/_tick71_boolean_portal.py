# -*- coding: utf-8 -*-
"""Tick #71: boolean recess into portal mass — different method vs additive cubes."""
import bpy
import os
import shutil
import bmesh
...
WORK = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V48_SNAP2.blend")
...
# Host mass for portal if missing
host = bpy.data.objects.get("PORTAL_ARCH_OUTER")
if not host:
    host = ensure_cube("PORTAL_ARCH_OUTER")
    set_size(host, 5.5, 1.6, 7.0, bottom_z=0.2, center_xy=(CX, 6.5))
    assign(host, MAT_STONE)

# Cutter for boolean difference (recess)
cutter = ensure_cube("PORTAL_BOOL_CUTTER")
set_size(cutter, 3.2, 2.2, 5.0, bottom_z=0.4, center_xy=(CX, 6.9))
# hide cutter from render after boolean
cutter.hide_render = True

# Apply scale to host/cutter
for o in (host, cutter):
    try:
...
# Ensure Boolean modifier DIFFERENCE
mod_name = "PORTAL_RECESS_BOOL"
existing = host.modifiers.get(mod_name)
if existing:
    host.modifiers.remove(existing)
...
# Dark inner shell slightly smaller for depth read
inner = ensure_cube("PORTAL_RECESS_SHELL")
set_size(inner, 3.0, 0.6, 4.7, bottom_z=0.45, center_xy=(CX, 6.55))
...
# Gold rim around recess
rim = ensure_cube("PORTAL_RECESS_RIM")
set_size(rim, 3.5, 0.25, 5.3, bottom_z=0.35, center_xy=(CX, 7.05))
assign(rim, MAT_GOLD)
try:
    for x in bpy.data.objects:
        x.select_set(False)
...
for o in bpy.data.objects:
    if o.type != "MESH" or o.hide_render:
        continue
    if o.name in ("PRES_GROUND", "LEVEL0_GROUND", "SCALE_HUMAN", "PORTAL_BOOL_CUTTER"):
        continue
...
maxx = maxy = maxz = -1e9
for o in bpy.data.objects:
    if o.type != "MESH" or o.hide_render or o.name in ("PRES_GROUND", "SCALE_HUMAN", "PORTAL_BOOL_CUTTER"):
        continue
...
print("BOOL_OK", host.name, cutter.name)
...
out_blend = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V49_BOOL.blend")
