# -*- coding: utf-8 -*-
"""Tick #73: tower front boolean window recesses + soft FP clamp to 24x19."""
import bpy
import os
import shutil
import bmesh
...
WORK = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V50_BOOLHALL.blend")
...
# Tower front face host
host = ensure_cube("TOWER_FRONT_MASS")
set_size(host, 6.5, 1.0, 22.0, bottom_z=8.0, center_xy=(CX, CY + 3.0))
assign(host, MAT_STONE)
apply_scale(host)

n = 0
for row, z in enumerate([12.0, 16.5, 21.0, 26.0]):
    for col, xoff in enumerate([-1.4, 1.4]):
        cut = ensure_cube(f"TOWER_BOOL_CUT_{row}_{col}")
        set_size(cut, 1.3, 1.6, 1.8, bottom_z=z, center_xy=(CX + xoff, CY + 3.2))
        apply_scale(cut)
        cut.hide_render = True
        mod_name = f"TOWER_WIN_BOOL_{row}_{col}"
        old = host.modifiers.get(mod_name)
        if old:
            host.modifiers.remove(old)
        ...
        try:
            ...
            pass
        shell = ensure_cube(f"TOWER_RECESS_SHELL_{row}_{col}")
        set_size(shell, 1.1, 0.4, 1.6, bottom_z=z + 0.1, center_xy=(CX + xoff, CY + 2.85))
        assign(shell, MAT_DARK)
        apply_scale(shell)
        n += 1

print("TOWER_BOOLS", n)

# Soft multi-pass clamp
for _ in range(2):
    for o in bpy.data.objects:
        if o.type != "MESH" or o.hide_render:
            continue
        if o.name in ("PRES_GROUND", "LEVEL0_GROUND", "SCALE_HUMAN") or "BOOL_CUT" in o.name:
            continue
...
maxx = maxy = maxz = -1e9
for o in bpy.data.objects:
    if o.type != "MESH" or o.hide_render or o.name in ("PRES_GROUND", "SCALE_HUMAN") or "BOOL_CUT" in o.name:
        continue
...
out_blend = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V51_BOOLTWR.blend")
