# -*- coding: utf-8 -*-
"""Tick #75: right wing boolean window recesses. Scale lock 24x19x38."""
import bpy
import os
import shutil
import bmesh
...
WORK = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V52_SNAP3.blend")
...
host = ensure_cube("WING_FRONT_MASS")
set_size(host, 6.0, 1.1, 7.5, bottom_z=0.3, center_xy=(8.0, 5.3))
assign(host, MAT_STONE)
apply_scale(host)

for i, x in enumerate([6.5, 8.0, 9.5]):
    cut = ensure_cube(f"WING_BOOL_CUT_{i}")
    set_size(cut, 1.2, 1.6, 2.0, bottom_z=5.2, center_xy=(x, 5.5))
    apply_scale(cut)
    cut.hide_render = True
    mod_name = f"WING_WIN_BOOL_{i}"
    ...
    try:
        ...
        pass
    shell = ensure_cube(f"WING_RECESS_SHELL_{i}")
    set_size(shell, 1.0, 0.45, 1.8, bottom_z=5.3, center_xy=(x, 5.15))
...
out_blend = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V53_BOOLWING.blend")
