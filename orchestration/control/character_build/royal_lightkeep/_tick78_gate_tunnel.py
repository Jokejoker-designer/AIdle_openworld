# -*- coding: utf-8 -*-
"""Tick #78: outer gate tunnel boolean through WALL65 front mass. Scale lock 24x19x38."""
import bpy
import os
import shutil
import bmesh
...
WORK = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V55_SNAP4.blend")
...
# Unified front wall host for tunnel (if WALL65 pieces exist, add mass bridge)
host = ensure_cube("GATE_TUNNEL_MASS")
set_size(host, 8.0, 1.4, 6.8, bottom_z=0.15, center_xy=(CX, 8.5))
assign(host, MAT_STONE)
apply_scale(host)

cut = ensure_cube("GATE_TUNNEL_CUT")
set_size(cut, 4.2, 2.5, 5.0, bottom_z=0.25, center_xy=(CX, 8.5))
apply_scale(cut)
cut.hide_render = True

mod_name = "GATE_TUNNEL_BOOL"
...
# Dark tunnel liner
liner = ensure_cube("GATE_TUNNEL_LINER")
set_size(liner, 3.8, 2.0, 4.6, bottom_z=0.3, center_xy=(CX, 8.2))
assign(liner, MAT_DARK)
apply_scale(liner)

# Gold inner arch rim
rim = ensure_cube("GATE_TUNNEL_RIM")
set_size(rim, 4.4, 0.3, 5.2, bottom_z=0.2, center_xy=(CX, 9.0))
assign(rim, MAT_GOLD)
apply_scale(rim)

# Bevel host
bmod = host.modifiers.get("BEVEL_SOFT")
if not bmod:
    bmod = host.modifiers.new(name="BEVEL_SOFT", type="BEVEL")
bmod.width = 0.08
bmod.segments = 2
bmod.limit_method = "ANGLE"

print("GATE_TUNNEL_OK")
...
out_blend = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V56_TUNNEL.blend")
