# -*- coding: utf-8 -*-
"""Tick #79: approach channel walls + tunnel-to-portal path reinforce. Scale lock 24x19x38."""
import bpy
import os
import shutil
import bmesh
...
WORK = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V56_TUNNEL.blend")
...
# Channel walls from outer gate toward portal
set_size(ensure_cube("CHANNEL_WALL_L"), 0.55, 4.5, 3.2, bottom_z=0.15, center_xy=(CX - 2.4, 7.2))
assign(bpy.data.objects["CHANNEL_WALL_L"], MAT_STONE)
set_size(ensure_cube("CHANNEL_WALL_R"), 0.55, 4.5, 3.2, bottom_z=0.15, center_xy=(CX + 2.4, 7.2))
assign(bpy.data.objects["CHANNEL_WALL_R"], MAT_STONE)

# Channel floor
set_size(ensure_cube("CHANNEL_FLOOR"), 4.0, 5.0, 0.15, bottom_z=0.1, center_xy=(CX, 7.0))
assign(bpy.data.objects["CHANNEL_FLOOR"], MAT_PATH)

# Gold rail caps on channel walls
set_size(ensure_cube("CHANNEL_CAP_L"), 0.6, 4.5, 0.15, bottom_z=3.3, center_xy=(CX - 2.4, 7.2))
assign(bpy.data.objects["CHANNEL_CAP_L"], MAT_GOLD)
set_size(ensure_cube("CHANNEL_CAP_R"), 0.6, 4.5, 0.15, bottom_z=3.3, center_xy=(CX + 2.4, 7.2))
assign(bpy.data.objects["CHANNEL_CAP_R"], MAT_GOLD)

# Inner threshold at portal base
set_size(ensure_cube("CHANNEL_THRESH"), 3.6, 0.8, 0.25, bottom_z=0.15, center_xy=(CX, 5.5))
assign(bpy.data.objects["CHANNEL_THRESH"], MAT_DARK)

for name in ("CHANNEL_WALL_L", "CHANNEL_WALL_R", "CHANNEL_FLOOR", "CHANNEL_CAP_L", "CHANNEL_CAP_R", "CHANNEL_THRESH"):
    apply_scale(bpy.data.objects[name])
...
out_blend = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V57_CHANNEL.blend")
