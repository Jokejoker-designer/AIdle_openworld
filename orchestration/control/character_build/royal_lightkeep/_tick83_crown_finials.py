# -*- coding: utf-8 -*-
"""Tick #83: crown finial ring densify around tower top. Scale lock 24x19x38."""
import bpy
import os
import shutil
import bmesh
...
WORK = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V60_SNAP6.blend")
...
# 8-point gold finial ring around crown
n = 0
r = 2.8
for i in range(8):
    ang = i * (math.pi * 2 / 8)
    x = CX + r * math.cos(ang)
    y = CY + r * math.sin(ang)
    o = ensure_cube(f"CROWN_RING_FINIAL_{i}")
    set_size(o, 0.28, 0.28, 1.1, bottom_z=35.5, center_xy=(x, y))
    assign(o, MAT_GOLD)
    apply_scale(o)
    base = ensure_cube(f"CROWN_RING_BASE_{i}")
    set_size(base, 0.45, 0.45, 0.35, bottom_z=35.2, center_xy=(x, y))
    assign(base, MAT_STONE)
    apply_scale(base)
    n += 2

# Center peak reinforce
set_size(ensure_cube("CROWN_PEAK_SPIRE"), 0.5, 0.5, 2.0, bottom_z=36.0, center_xy=(CX, CY))
assign(bpy.data.objects["CROWN_PEAK_SPIRE"], MAT_ROOF)
apply_scale(bpy.data.objects["CROWN_PEAK_SPIRE"])
set_size(ensure_cube("CROWN_PEAK_GOLD"), 0.28, 0.28, 0.9, bottom_z=37.5, center_xy=(CX, CY))
assign(bpy.data.objects["CROWN_PEAK_GOLD"], MAT_GOLD)
apply_scale(bpy.data.objects["CROWN_PEAK_GOLD"])
n += 2

print("CROWN_FINIALS", n)
...
out_blend = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V61_CROWN2.blend")
