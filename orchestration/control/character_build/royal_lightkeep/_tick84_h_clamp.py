# -*- coding: utf-8 -*-
"""Tick #84 P1: hard height clamp to 38.2m after crown densify."""
import bpy
import os
import shutil
...
WORK = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V61_CROWN2.blend")
...
# Prefer shifting crown finials down first
for o in bpy.data.objects:
    if o.type != "MESH" or o.hide_render:
        continue
    if o.name.startswith(("CROWN_RING_", "CROWN_PEAK_", "GOLD_FINIAL", "CROWN_CENTER")):
        top = o.location.z + o.dimensions.z / 2.0
        if top > H_MAX:
            o.location.z -= (top - H_MAX + 0.05)

# Global H + footprint clamp
for _ in range(2):
    for o in bpy.data.objects:
        if o.type != "MESH" or o.hide_render or o.name in SKIP:
            continue
        if "BOOL_CUT" in o.name or o.name.endswith("_CUT"):
            continue
        top = o.location.z + o.dimensions.z / 2.0
        if top > H_MAX:
...
out_blend = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V62_HCLAMP.blend")
