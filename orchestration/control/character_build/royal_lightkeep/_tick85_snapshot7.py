# -*- coding: utf-8 -*-
"""Tick #85: inventory + deviation after H-clamp. Re-render."""
import bpy
import os
import shutil
...
WORK = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V62_HCLAMP.blend")
...
maxx = maxy = maxz = -1e9
for o in meshes:
    if o.name in SKIP or "BOOL_CUT" in o.name or o.name.endswith("_CUT"):
        continue
...
print("BOUNDS", round(bw, 2), round(bd, 2), round(bh, 2), "Zmax", round(maxz, 2))
...
with open(os.path.join(BASE, "INTERMEDIATE_DEVIATION_REPORT.md"), "w", encoding="utf-8") as f:
    f.write(f"""# INTERMEDIATE DEVIATION REPORT — tick #85

**Asset:** ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01
**Stamp:** {STAMP}
**File:** PASS8_V63_SNAP7 / PASS1D
...
## Matches
- Scale lock restored (H clamp after crown densify)
- Entry tunnel + channel + multi-zone booleans
- Crown finial ring, copestones, setbacks, gallery, arcade
...
## Overall ~8.05 — not FINAL until Human overlay
Scale healthy. Modular plateau remains vs carved gothic mockup.
""")
...
out_blend = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V63_SNAP7.blend")
