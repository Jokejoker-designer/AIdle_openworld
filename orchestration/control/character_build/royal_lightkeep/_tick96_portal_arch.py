# -*- coding: utf-8 -*-
"""Tick #96: portal pointed-arch densify (stepped voussoirs) + inventory snapshot.
Continue PASS8_V73_ROOF_HIP. Scale lock 24x19x38. No FINAL claim."""
import bpy
import os
import shutil
import bmesh
from datetime import datetime
from mathutils import Vector
from collections import Counter

BASE = r"E:\AIdle_openworld\orchestration\control\character_build\royal_lightkeep"
WORK = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V73_ROOF_HIP.blend")
LOOP = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS1D.blend")
STAMP = datetime.now().strftime("%Y%m%d_%H%M%S")

bpy.ops.wm.open_mainfile(filepath=WORK)
print("OPEN", bpy.data.filepath)

backup = os.path.join(BASE, f"ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_BACKUP_LOOP_{STAMP}.blend")
bpy.ops.wm.save_as_mainfile(filepath=backup, copy=True)
print("BACKUP", backup)


def mesh_local_size(obj):
    me = obj.data
    xs = [v.co.x for v in me.vertices]
    ys = [v.co.y for v in me.vertices]
    zs = [v.co.z for v in me.vertices]
    return Vector((max(xs) - min(xs) or 1e-6, max(ys) - min(ys) or 1e-6, max(zs) - min(zs) or 1e-6))


def set_size(obj, sx, sy, sz, bottom_z=None, center_xy=None):
    loc = mesh_local_size(obj)
    obj.scale = Vector((sx / loc.x, sy / loc.y, sz / loc.z))
    bpy.context.view_layer.update()
    if center_xy is not None:
        obj.location.x, obj.location.y = center_xy
    if bottom_z is not None:
        obj.location.z = bottom_z + obj.dimensions.z / 2.0


def ensure_cube(name):
    o = bpy.data.objects.get(name)
    if o:
        o.hide_render = False
        o.hide_viewport = False
        return o
    mesh = bpy.data.meshes.new(name)
    bm = bmesh.new()
    bmesh.ops.create_cube(bm, size=2.0)
    bm.to_mesh(mesh)
    bm.free()
    o = bpy.data.objects.new(name, mesh)
    bpy.context.scene.collection.objects.link(o)
    return o


def make_mat(name, color, rough=0.7, metal=0.0):
    m = bpy.data.materials.get(name)
    if not m:
        m = bpy.data.materials.new(name)
        m.use_nodes = True
    if m.use_nodes:
        bsdf = m.node_tree.nodes.get("Principled BSDF")
        if bsdf:
            bsdf.inputs["Base Color"].default_value = (*color, 1.0)
            bsdf.inputs["Roughness"].default_value = rough
            if "Metallic" in bsdf.inputs:
                bsdf.inputs["Metallic"].default_value = metal
    return m


def assign(obj, mat):
    if not obj or obj.type != "MESH":
        return
    if obj.data.materials:
        obj.data.materials[0] = mat
    else:
        obj.data.materials.append(mat)


def apply_scale(o):
    try:
        for x in bpy.data.objects:
            x.select_set(False)
        o.select_set(True)
        bpy.context.view_layer.objects.active = o
        bpy.ops.object.transform_apply(location=False, rotation=True, scale=True)
    except Exception as e:
        print("APPLY_ERR", o.name, e)


def soft_bevel(o, width=0.04, segments=2):
    if o.type != "MESH":
        return
    mod = None
    for m in o.modifiers:
        if m.type == "BEVEL":
            mod = m
            break
    if mod is None:
        mod = o.modifiers.new(name="BEVEL_SOFT", type="BEVEL")
    mod.width = width
    mod.segments = segments
    mod.limit_method = "ANGLE"
    try:
        mod.angle_limit = 0.7
    except Exception:
        pass


MAT_STONE = make_mat("MAT_LIMESTONE", (0.82, 0.78, 0.70), 0.78, 0.0)
MAT_DARK = make_mat("MAT_FOUNDATION_DARK", (0.18, 0.16, 0.14), 0.90, 0.0)
MAT_GOLD = make_mat("MAT_GOLD_TRIM", (0.90, 0.68, 0.22), 0.22, 0.95)
MAT_WOOD = make_mat("MAT_WOOD", (0.35, 0.22, 0.12), 0.75, 0.0)

CX = 1.0
X_MIN, X_MAX = -11.5, 12.5
Y_MIN, Y_MAX = -8.5, 10.5
H_MAX = 38.2
n = 0

# Pointed arch approximation: ascending stepped voussoir pairs (gothic silhouette)
# Width narrows as height rises → pointed look in front view
y_face = 7.15
for i in range(10):
    t = i / 9.0
    # half-width shrinks from 1.9 to 0.15 (point)
    half_w = 1.9 * (1.0 - t) + 0.12
    z = 0.5 + i * 0.55
    h = 0.5
    # left voussoir
    L = ensure_cube(f"PORTAL_VOUSSOIR_L_{i}")
    set_size(L, 0.55, 0.45, h, bottom_z=z, center_xy=(CX - half_w, y_face))
    assign(L, MAT_STONE if i % 2 == 0 else MAT_DARK)
    apply_scale(L)
    soft_bevel(L, 0.02)
    # right voussoir
    R = ensure_cube(f"PORTAL_VOUSSOIR_R_{i}")
    set_size(R, 0.55, 0.45, h, bottom_z=z, center_xy=(CX + half_w, y_face))
    assign(R, MAT_STONE if i % 2 == 0 else MAT_DARK)
    apply_scale(R)
    soft_bevel(R, 0.02)
    n += 2

# Apex keystone stack
for i, (w, z) in enumerate([(0.7, 5.7), (0.5, 6.1), (0.35, 6.45)]):
    k = ensure_cube(f"PORTAL_APEX_{i}")
    set_size(k, w, 0.5, 0.4, bottom_z=z, center_xy=(CX, y_face + 0.05))
    assign(k, MAT_GOLD if i == 2 else MAT_STONE)
    apply_scale(k)
    n += 1

# Outer hood mold (frame around arch)
hood = ensure_cube("PORTAL_HOOD_MOLD")
set_size(hood, 5.0, 0.35, 6.8, bottom_z=0.3, center_xy=(CX, y_face + 0.25))
assign(hood, MAT_DARK)
apply_scale(hood)
soft_bevel(hood, 0.05)
n += 1

# Gold fillet on hood
fil = ensure_cube("PORTAL_HOOD_GOLD")
set_size(fil, 5.2, 0.18, 0.2, bottom_z=6.9, center_xy=(CX, y_face + 0.3))
assign(fil, MAT_GOLD)
apply_scale(fil)
n += 1

# Inner dark void face (deeper recess plane)
void = ensure_cube("PORTAL_VOID_FACE")
set_size(void, 2.8, 0.2, 5.0, bottom_z=0.45, center_xy=(CX, 6.7))
assign(void, MAT_DARK)
apply_scale(void)
n += 1

# Reinforce gate leaves
for side, xoff in (("L", -0.9), ("R", 0.9)):
    g = ensure_cube(f"PORTAL_GATE_{side}")
    set_size(g, 1.55, 0.16, 4.2, bottom_z=0.4, center_xy=(CX + xoff, 6.45))
    assign(g, MAT_WOOD)
    apply_scale(g)
    n += 1

# Side colonnettes
for side, x in (("L", CX - 2.5), ("R", CX + 2.5)):
    c = ensure_cube(f"PORTAL_COLONNETTE_{side}")
    set_size(c, 0.35, 0.35, 5.5, bottom_z=0.3, center_xy=(x, y_face))
    assign(c, MAT_STONE)
    apply_scale(c)
    soft_bevel(c, 0.04)
    cap = ensure_cube(f"PORTAL_COLONNETTE_CAP_{side}")
    set_size(cap, 0.5, 0.5, 0.25, bottom_z=5.7, center_xy=(x, y_face))
    assign(cap, MAT_GOLD)
    apply_scale(cap)
    n += 2

print("CREATED_OR_UPDATED", n)

# Soft clamp
for o in bpy.data.objects:
    if o.type != "MESH" or o.hide_render:
        continue
    if o.name in ("PRES_GROUND", "LEVEL0_GROUND", "SCALE_HUMAN") or "CUT" in o.name or "CUTTER" in o.name or "NICHE" in o.name:
        continue
    corners = [o.matrix_world @ Vector(c) for c in o.bound_box]
    minx = min(c.x for c in corners)
    maxx = max(c.x for c in corners)
    miny = min(c.y for c in corners)
    maxy = max(c.y for c in corners)
    if minx < X_MIN:
        o.location.x += (X_MIN - minx)
    if maxx > X_MAX:
        o.location.x += (X_MAX - maxx)
    if miny < Y_MIN:
        o.location.y += (Y_MIN - miny)
    if maxy > Y_MAX:
        o.location.y += (Y_MAX - maxy)
    top = o.location.z + o.dimensions.z / 2.0
    if top > H_MAX:
        o.location.z -= (top - H_MAX)

minx = miny = minz = 1e9
maxx = maxy = maxz = -1e9
mesh_n = 0
bool_n = 0
beveled = 0
meshes = []
for o in bpy.data.objects:
    if o.type != "MESH":
        continue
    if any(m.type == "BOOLEAN" for m in o.modifiers):
        bool_n += 1
    if any(m.type == "BEVEL" for m in o.modifiers):
        beveled += 1
    if o.hide_render:
        continue
    if o.name in ("PRES_GROUND", "SCALE_HUMAN") or "CUT" in o.name or "CUTTER" in o.name or "NICHE" in o.name:
        continue
    mesh_n += 1
    meshes.append(o)
    for corner in o.bound_box:
        w = o.matrix_world @ Vector(corner)
        minx = min(minx, w.x)
        maxx = max(maxx, w.x)
        miny = min(miny, w.y)
        maxy = max(maxy, w.y)
        minz = min(minz, w.z)
        maxz = max(maxz, w.z)
bw, bd, bh = maxx - minx, maxy - miny, maxz - minz
pref = Counter(o.name.split("_")[0] for o in meshes)
top = pref.most_common(12)
cams = sum(1 for o in bpy.data.objects if o.type == "CAMERA")
lights = sum(1 for o in bpy.data.objects if o.type == "LIGHT")
print("BOUNDS", round(bw, 2), round(bd, 2), round(bh, 2), "Z", round(minz, 2), round(maxz, 2), "MESHES", mesh_n, "BOOL", bool_n)

state = f"""# AUTONOMOUS BUILD STATE — ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01

**Updated:** 2026-07-27 (job `019f9eec8b6f` tick **#96**, 5m)  
**Asset ID:** `ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01`  

| Field | Value |
|-------|--------|
| **ASSET_FINAL_COMPLETE** | **false** |
| **accepted (user)** | **false** |
| **Current** | **PASS8 V74 PORTAL_ARCH** |
| **Scale** | FP **{bw:.1f}×{bd:.1f}** (≤24×19) · top Z **{maxz:.1f}** |
| **Scheduler** | `019f9eec8b6f` every **5 min** |
| **Pipeline** | IN_PROGRESS (portal pointed-arch densify) |

---

## Tick #96 — executed (P1)

### Edits
1. **Pointed arch** — 10-step voussoir pairs (L/R taper to apex)
2. **Apex stack** + hood mold + gold fillet + void face
3. **Colonnettes** + gate leaf reinforce

### Inventory
- Meshes: **{mesh_n}** · Bool: **{bool_n}** · Bevel: **{beveled}**
- Cams: **{cams}** · Lights: **{lights}**
- Top: {", ".join(f"{k}:{v}" for k, v in top)}

### Files
| Role | Path |
|------|------|
| Work | `ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V74_PORTAL_ARCH.blend` |
| Loop / candidate | `PASS1D.blend` / `FINAL.blend` (synced) |
| Backup | `...BACKUP_LOOP_{STAMP}.blend` |
| Proofs | `renders_final/*` + work proofs |

### Bounds
**{bw:.1f} × {bd:.1f} × {bh:.1f}** m · Z {minz:.1f}..**{maxz:.1f}**

### Internal scores
| Axis | Score |
|------|-------|
| Scale | **9.0** |
| Portal / arch | ~8.45 |
| Front silhouette | ~8.35 |
| Gothic fidelity | ~8.3 |
| Overall | **~8.45** |

### Verdict
Not FINAL. Portal pointed language improved; D1 modular core still primary Human blocker.  
Next: mid-tower window string densify OR keep declutter path.
"""

loop_state = f"""# BUILD LOOP STATE

**Updated:** 2026-07-27 · tick **#96** · job `019f9eec8b6f` (5m)  
**ASSET_FINAL_COMPLETE:** false · **Current:** PASS8_V74_PORTAL_ARCH  

## Tick #96 (P1)
- Pointed arch voussoirs (10 steps) + apex/hood/colonnettes
- Inventory: meshes {mesh_n}, bool {bool_n}
- Bounds {bw:.1f}×{bd:.1f}×{bh:.1f}
- Backup: BACKUP_LOOP_{STAMP}
- Saved: PASS8_V74_PORTAL_ARCH → PASS1D / FINAL

## Next
Tower mid window string densify or declutter; Human overlay when closer
"""

dev = f"""# INTERMEDIATE DEVIATION REPORT — tick #96

**Asset:** ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01
**Stamp:** {STAMP}
**File:** PASS8_V74_PORTAL_ARCH / PASS1D
**ASSET_FINAL_COMPLETE:** false
**accepted (user):** false

## Inventory
- Mesh objects (visible): **{mesh_n}**
- Boolean hosts: **{bool_n}** · Bevel: **{beveled}**
- Cameras: **{cams}** · Lights: **{lights}**
- Top prefixes: {", ".join(f"{k}:{v}" for k, v in top)}

## Sheet targets
- Footprint 24×19 m · Height 38 m · Wall 6.5 m
- Achieved: **{bw:.2f}×{bd:.2f}×{bh:.2f}** m · Z {minz:.2f}..{maxz:.2f}

## Matches
- Stepped pointed-arch portal (voussoir taper)
- Hood mold + colonnettes + gates
- Prior roof hip / curtain / shaft stack retained
- Scale lock held

## Gaps vs mockup (blocking Human accept)
| ID | Pri | Gap |
|----|-----|-----|
| D1 | P1 | Core modular-box language still dominant |
| D2 | P1 | Roof hips still plate-rotated not organic |
| D3 | P2 | Arch is stepped-cubes not true curved mesh |
| D4 | P2 | High mesh count residual clutter risk |
| D5 | P3 | UV/LOD not authored |

## Overall ~8.45 — not FINAL until Human overlay
Portal improved; still not Human-accept ready.
"""

with open(os.path.join(BASE, "AUTONOMOUS_BUILD_STATE.md"), "w", encoding="utf-8") as f:
    f.write(state)
with open(os.path.join(BASE, "PASS1D_LOOP_STATE.md"), "w", encoding="utf-8") as f:
    f.write(loop_state)
with open(os.path.join(BASE, "INTERMEDIATE_DEVIATION_REPORT.md"), "w", encoding="utf-8") as f:
    f.write(dev)
print("WROTE_STATE")

flag = os.path.join(BASE, "ASSET_FINAL_COMPLETE.flag")
if os.path.isfile(flag):
    try:
        os.remove(flag)
    except Exception:
        pass

scene = bpy.context.scene
try:
    scene.render.engine = "BLENDER_EEVEE_NEXT"
except Exception:
    try:
        scene.render.engine = "BLENDER_EEVEE"
    except Exception:
        scene.render.engine = "CYCLES"
scene.render.resolution_x = 1280
scene.render.resolution_y = 960

out_blend = os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_PASS8_V74_PORTAL_ARCH.blend")
bpy.ops.wm.save_as_mainfile(filepath=out_blend)
shutil.copy2(out_blend, LOOP)
shutil.copy2(out_blend, os.path.join(BASE, "ROYAL_LIGHTKEEP_WATCHTOWER_BARRACKS_01_FINAL.blend"))
print("SAVED V74")

out_final = os.path.join(BASE, "renders_final")
out_work = os.path.join(BASE, "renders_pass1d")
for d in (out_final, out_work):
    os.makedirs(d, exist_ok=True)

jobs = [
    ("CAM_01_FRONT", out_work, "current_front_work.png"),
    ("CAM_05_FRONT_3Q", out_work, "current_front_3q_work.png"),
    ("CAM_06_REAR_3Q", out_work, "current_rear_3q_work.png"),
    ("CAM_TOP_PLAN", out_work, "current_top_plan_work.png"),
    ("CAM_01_FRONT", out_final, "final_front.png"),
    ("CAM_05_FRONT_3Q", out_final, "final_front_3q.png"),
    ("CAM_02_REAR", out_final, "final_rear.png"),
    ("CAM_03_LEFT", out_final, "final_left.png"),
    ("CAM_04_RIGHT", out_final, "final_right.png"),
    ("CAM_06_REAR_3Q", out_final, "final_rear_3q.png"),
    ("CAM_TOP_PLAN", out_final, "final_top.png"),
]
for cam, dest, fn in jobs:
    c = bpy.data.objects.get(cam)
    if not c:
        print("MISS_CAM", cam)
        continue
    scene.camera = c
    scene.render.filepath = os.path.join(dest, fn)
    bpy.ops.render.render(write_still=True)
    print("OK", fn)

print("TICK96_DONE")
