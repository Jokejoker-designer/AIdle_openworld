...
## Writes under orchestration/evidence/h1_consolidate_001/002 only — never patches product.
...
const EVIDENCE_ABS := "E:/AIdle_openworld/orchestration/evidence/h1_consolidate_001/002"
...
func _initialize() -> void:
	print("[H1C_C2_HEADED] start real_input=true no_api_fallback=true wave=C2")
	print("[H1C_C2_HEADED] evidence=%s" % EVIDENCE_ABS)
	...
	if art != null and art.has_method("set_world_meta_path_override"):
		art.call("set_world_meta_path_override", "user://h1c_c2_isolated/world_meta.cfg")
	...
	if not _product_key_p_present:
		...
		print("[H1C_C2_HEADED] WARN product KEY_P missing — companion-led path still primary")
...
func _teardown_clean() -> void:
	...
	await process_frame
	await process_frame
	print("[H1C_C2_HEADED] teardown_clean done")
...
func _run_state_matrix(tag: String, w: int, h: int) -> void:
	...
	print("[H1C_C2_HEADED] viewport %s matrix done captures_so_far=%d" % [tag, _captures.size()])
...
func _set_window(w: int, h: int) -> void:
	...
	print("[H1C_C2_HEADED] window=%dx%d" % [w, h])
...
func _capture(filename: String, expect_w: int, expect_h: int, state: String, extra: Dictionary = {}) -> void:
	...
	print(
		"[H1C_C2_HEADED] CAPTURED file=%s %dx%d sha=%s state=%s"
...
func _write_runtime_manifest() -> void:
	...
	for s in REQUIRED_STATES:
		if not states_seen.has(s):
			missing.append(s)
	var meta := {
		"schema": "h1_consolidate_001_c2_visual_claim_meta/1.0",
...
func _finish() -> void:
	...
	print(
		"[H1C_C2_HEADED] done passed=%d failed=%d captures=%d need>=%d forbidden_hits=%d"
		% [_passed, _failed, _captures.size(), need, _forbidden_hits.size()]
	)
	if _failed == 0 and _captures.size() >= need:
		print("AIDLE_H1C_C2_HEADED=PASS captures=%d" % _captures.size())
		quit(0)
	else:
		print("AIDLE_H1C_C2_HEADED=FAIL failed=%d captures=%d" % [_failed, _captures.size()])
		quit(1)
