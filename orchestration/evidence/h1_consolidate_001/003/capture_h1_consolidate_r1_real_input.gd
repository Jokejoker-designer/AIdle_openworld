## H1-CONSOLIDATE-001 C2 headed real-input capture (VERIFY_ONLY evidence lease only).
## Writes under orchestration/evidence/h1_consolidate_001/003 only — never patches product.
...
const EVIDENCE_ABS := "E:/AIdle_openworld/orchestration/evidence/h1_consolidate_001/003"
...
func _initialize() -> void:
	print("[H1C_R1_HEADED] start real_input=true no_api_fallback=true wave=R1")
	print("[H1C_R1_HEADED] evidence=%s" % EVIDENCE_ABS)
	...
	if art != null and art.has_method("set_world_meta_path_override"):
		art.call("set_world_meta_path_override", "user://h1c_r1_isolated/world_meta.cfg")
	...
	if not _product_key_p_present:
		...
		print("[H1C_R1_HEADED] WARN product KEY_P missing — companion-led path still primary")
...
func _teardown_clean() -> void:
	...
	await process_frame
	await process_frame
	print("[H1C_R1_HEADED] teardown_clean done")
...
func _run_state_matrix(tag: String, w: int, h: int) -> void:
	...
	print("[H1C_R1_HEADED] viewport %s matrix done captures_so_far=%d" % [tag, _captures.size()])
...
func _install_banner() -> void:
	...
	_banner.text = "H1-CONSOLIDATE R1 real-input evidence"
	_banner_layer.add_child(_banner)


func _set_banner(text: String) -> void:
	if _banner != null:
		_banner.text = "H1C-R1 | " + text
...
func _set_window(w: int, h: int) -> void:
	...
	print("[H1C_R1_HEADED] window=%dx%d" % [w, h])
...
func _capture(filename: String, expect_w: int, expect_h: int, state: String, extra: Dictionary = {}) -> void:
	...
	print(
		"[H1C_R1_HEADED] CAPTURED file=%s %dx%d sha=%s state=%s"
...
func _write_runtime_manifest() -> void:
	...
	for s in REQUIRED_STATES:
		if not states_seen.has(s):
			missing.append(s)
	var meta := {
		"schema": "h1_consolidate_001_r1_visual_claim_meta/1.0",
		"work_order": "WO-H1-CONSOLIDATE-001-RUNTIME-ROOT-CORRECTION-002",
		"wave": "R1",
		"directive_id": 76,
		...
		"residuals": [
			"R1: wireframe/hologram/materializing captured via advance_stage presentation walk (same stages as handle_player_confirm) for visual distinctness; commit uses KEY_ENTER only",
			"R1: proposal→preview uses Proposal Card confirm_requested product signal; begin_companion_led_build only if card path fails",
...
func _finish() -> void:
	...
	print(
		"[H1C_R1_HEADED] done passed=%d failed=%d captures=%d need>=%d forbidden_hits=%d"
		% [_passed, _failed, _captures.size(), need, _forbidden_hits.size()]
	)
	if _failed == 0 and _captures.size() >= need:
		print("AIDLE_H1C_R1_HEADED=PASS captures=%d" % _captures.size())
		quit(0)
	else:
		print("AIDLE_H1C_R1_HEADED=FAIL failed=%d captures=%d" % [_failed, _captures.size()])
		quit(1)
