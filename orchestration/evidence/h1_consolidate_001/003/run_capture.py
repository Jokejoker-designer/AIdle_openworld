#!/usr/bin/env python3
"""H1-CONSOLIDATE-001 R1 headed real-input capture runner (VERIFY_ONLY evidence lease).

Fresh fail-closed harness under evidence/h1_consolidate_001/003 only.
...
EVIDENCE = ROOT / "orchestration" / "evidence" / "h1_consolidate_001" / "003"
SCRIPT = EVIDENCE / "capture_h1_consolidate_r1_real_input.gd"
...
def main() -> int:
    ...
    user_data = Path(os.environ.get("TEMP", "/tmp")) / f"h1c_r1_userdata_{ts}"
    ...
    marker_pass = "AIDLE_H1C_R1_HEADED=PASS" in combined
    ...
    manifest = {
        "schema": "h1_consolidate_001_r1_evidence_manifest/1.0",
