#!/usr/bin/env python3
"""H1-CONSOLIDATE-001 W2 headless smoke runner (VERIFY_ONLY evidence lease 004).

Runs primary Manual Build smoke + H1/P2E/Control/G3/G4/Block-DNA regressions.
Does not patch product. Writes only under evidence/h1_consolidate_001/004/smokes/.
"""
...
EVIDENCE = ROOT / "orchestration" / "evidence" / "h1_consolidate_001" / "004"
...
GODOT_SMOKES = [
    (
        ...
        r"AIDLE_H1_HUMAN_UX_MANUAL_BUILD_SMOKE=PASS checks=9",
    ),
    (
        "LOOKUP",
...
def main() -> int:
    ...
    results: list[dict] = []
    t0 = time.time()
    print(f"=== W2 SMOKES START {time.strftime('%Y-%m-%dT%H:%M:%S')} ===")
    ...
    for sid, script, marker in GODOT_SMOKES:
        ...
        # Honest residual: H1_FLOW/H1_CHROME may FAIL on Small Build tscn residual (W1-RES-01).
        ...
        results.append(
            {
                ...
                "marker_ok": marker_ok,
                "error_lines": err_n,
                "log": f"orchestration/evidence/h1_consolidate_001/004/smokes/{sid}.log",
                "marker": marker,
                "detail": next(
        ...
        results.append(
            {
                ...
                "marker_ok": ok,
                "error_lines": 0,
                "log": "orchestration/evidence/h1_consolidate_001/004/smokes/block_dna.log",
        ...
        results.append(
            {
                ...
                "marker_ok": ok,
                "error_lines": 0,
                "log": "orchestration/evidence/h1_consolidate_001/004/smokes/control_1b_fixtures.log",
    ...
    # Overall smoke_all_pass excludes known residual chrome/flow Small Build fails if primary PASS.
    residual_ids = {"H1_FLOW", "H1_CHROME"}
    hard_pass = all(r["pass"] for r in results if r["id"] not in residual_ids)
    ...
    summary = {
        ...
        "overall_pass_excluding_known_chrome_residual": hard_pass and bool(primary and primary["pass"]),
        "known_residuals": ["W1-RES-01 Small Build tscn / H1 chrome+flow string assert"],
        "checks": results,
    ...
    print("HARD_PASS_EXCL_CHROME_RESIDUAL", hard_pass and bool(primary and primary["pass"]))
    print(f"=== W2 SMOKES DONE elapsed={summary['elapsed_s']}s ===")
    # Exit 0 if primary + non-residual suite pass (chrome residual allowed).
    return 0 if (primary and primary["pass"] and hard_pass) else 1
