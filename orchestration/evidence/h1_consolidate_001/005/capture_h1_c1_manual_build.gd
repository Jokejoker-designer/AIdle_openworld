## H1-CONSOLIDATE-001 C1 headed Manual Build capture (VERIFY_ONLY evidence lease 005).
## Dual-res 1280×720 + 868×517. Proves UX/Manual Build gates; never patches product.
## Marker: AIDLE_H1C_C1_HEADED=PASS|FAIL
extends SceneTree

const MAIN_SCENE := "res://scenes/main/main.tscn"
const EVIDENCE_ABS := "E:/AIdle_openworld/orchestration/evidence/h1_consolidate_001/005"
...
func _initialize() -> void:
	print("[H1C_C1_HEADED] start wave=C1 directive=79 real_input=true")
	print("[H1C_C1_HEADED] evidence=%s" % EVIDENCE_ABS)
	...
	if art != null and art.has_method("set_world_meta_path_override"):
		art.call("set_world_meta_path_override", "user://h1c_c1_isolated/world_meta.cfg")
...
func _run_state_matrix(tag: String, w: int, h: int) -> void:
	...
	# C0 closed F02: scene + runtime both Manual Build; residual Small Build is FAIL.
	...
	var tscn_has_manual := tscn_text.find("Manual Build") >= 0
	var tscn_residual_small_build := tscn_text.find("Small Build") >= 0
	if tscn_residual_small_build or not tscn_has_manual:
		_fail(
			"manual_build_label_tscn",
			"viewport=%s residual_small=%s has_manual=%s"
			% [tag, str(tscn_residual_small_build), str(tscn_has_manual)]
		)
	_gate(
		"manual_build_label_runtime_gd",
		label_src_manual and tscn_has_manual and not tscn_residual_small_build,
		tag
	)
	_set_banner(
		"manual_build_enter | ok=%s | Manual Build tscn=%s residual_Small=%s"
		% [str(bool(boot.get("ok", false))), str(tscn_has_manual), str(tscn_residual_small_build)]
	)
	for i in range(10):
		await process_frame
	var st_enter: Dictionary = _ba.call("get_active_state") as Dictionary if _ba else {}
	# Intentional LMB: origin/manual enter must not be confirmable before place.
	var can_conf_before_place := true
	if _ba != null and _ba.has_method("can_confirm"):
		can_conf_before_place = bool(_ba.call("can_confirm"))
	if can_conf_before_place:
		_fail("confirmable_before_intentional_lmb", "viewport=%s enter_state" % tag)
	_gate("not_confirmable_before_lmb_enter", not can_conf_before_place, tag)
	await _capture(
		...
		"manual_build_enter",
		{
			...
			"tscn_has_manual_build": tscn_has_manual,
			"tscn_residual_small_build": tscn_residual_small_build,
			"can_confirm_before_intentional_lmb": can_conf_before_place,
	...
	# ── 9) intentional LMB then confirm through World Commit ──────────────
	if _router != null and _router.has_method("request_context"):
		_router.call("request_context", "build")
	for i in range(4):
		await process_frame
	...
	for i in range(6):
		await process_frame
	var can_before_lmb := true
	if _ba != null and _ba.has_method("can_confirm"):
		can_before_lmb = bool(_ba.call("can_confirm"))
	if can_before_lmb:
		_fail("confirmable_before_intentional_lmb", "viewport=%s pre_place" % tag)
	var pl: Dictionary = {}
	if _ba != null and _ba.has_method("place_highlighted_module"):
		pl = _ba.call("place_highlighted_module") as Dictionary
		_log_input(
			"preview",
			"place_highlighted_module",
			{"ok": bool(pl.get("ok", false)), "preview_only": bool(pl.get("preview_only", false))}
		)
	for i in range(10):
		await process_frame
	var can_after_lmb := false
	if _ba != null and _ba.has_method("can_confirm"):
		can_after_lmb = bool(_ba.call("can_confirm"))
	if not can_after_lmb and bool(pl.get("ok", false)):
		_fail("not_confirmable_after_intentional_lmb", "viewport=%s place=%s" % [tag, str(pl)])
	_gate(
		"intentional_lmb_before_confirm",
		(not can_before_lmb) and (can_after_lmb or not bool(pl.get("ok", false))),
		tag
	)
	...
	_set_banner(
		"confirm | lmb_gate=%s→%s ok=%s status=%s committed=%d"
		% [str(can_before_lmb), str(can_after_lmb), str(confirm_ok), status, committed_after]
	)
	await process_frame
	await _capture(
		"confirm_%s.png" % tag,
		...
		"confirm",
		{
			...
			"can_confirm_before_intentional_lmb": can_before_lmb,
			"can_confirm_after_intentional_lmb": can_after_lmb,
			"place_ok": bool(pl.get("ok", false)),
			"preview_only_place": bool(pl.get("preview_only", false)),
			"via": str(conf.get("via", receipt.get("via", "handle_player_confirm"))),
			"input_sequence": [
				"begin_manual_build",
				"force_cursor",
				"assert not can_confirm",
				"place_highlighted_module (intentional LMB)",
				"assert can_confirm",
	...
	print("[H1C_C1_HEADED] viewport %s matrix done captures=%d fails=%d" % [tag, _captures.size(), _failed])
...
func _teardown_clean() -> void:
	...
	await process_frame
	await process_frame
	print("[H1C_C1_HEADED] teardown_clean done")
...
func _log_input(kind: String, label: String, extra: Dictionary) -> void:
	...
	for k in extra.keys():
		e[k] = extra[k]
	_input_log.append(e)
	print("[H1C_C1_INPUT] %s %s %s" % [kind, label, str(extra)])


func _gate(name: String, ok: bool, viewport: String) -> void:
	_gate_log.append({"gate": name, "ok": ok, "viewport": viewport})
	print("[H1C_C1_GATE] %s ok=%s viewport=%s" % [name, str(ok), viewport])
...
func _install_banner() -> void:
	...
	_banner.text = "H1-CONSOLIDATE C1 Manual Build evidence"
	_banner_layer.add_child(_banner)


func _set_banner(text: String) -> void:
	if _banner != null:
		_banner.text = "H1C-C1 | " + text
...
func _capture(filename: String, w: int, h: int, state: String, meta: Dictionary) -> void:
	...
	if digest != "" and _sha_seen.has(digest):
		...
		print("[H1C_C1_HEADED] WARN duplicate_sha %s == %s" % [filename, str(_sha_seen[digest])])
	elif digest != "":
	...
	print("[H1C_C1_HEADED] captured %s %dx%d sha=%s" % [filename, iw, ih, digest.substr(0, 12)])


func _write_runtime_manifest() -> void:
	var meta := {
		"schema": "h1_consolidate_001_c1_visual_claim_meta/1.0",
		"wave": "C1",
		"directive_id": 79,
		"art_style_id_active": _art_style_id,
		"capture_source": "godot_headed",
		...
		"tscn_residual_small_build_noted": false,
	}
...
func _finish() -> void:
	...
	if all_ok:
		print("AIDLE_H1C_C1_HEADED=PASS checks=%d captures=%d gates=%d" % [_passed, _captures.size(), _gate_log.size()])
		quit(0)
	else:
		for f in _failures:
			printerr("[FAIL] %s" % f)
		print(
			"AIDLE_H1C_C1_HEADED=FAIL failed=%d passed=%d captures=%d expected_png=%d gates_ok=%s"
