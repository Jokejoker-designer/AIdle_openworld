## UCBV-001 C4 clean dual-res headed evidence (VERIFY_ONLY lease evidence/ucbv_001/002 only).
## Directive 91. Normal play — no diagnostic banner wall.
...
## Marker: AIDLE_UCBV001_C4_HEADED=PASS|FAIL
extends SceneTree

const MAIN_SCENE := "res://scenes/main/main.tscn"
const EVIDENCE_ABS := "E:/AIdle_openworld/orchestration/evidence/ucbv_001/002"
const SELF_SCRIPT := "E:/AIdle_openworld/orchestration/evidence/ucbv_001/002/capture_ucbv_c4_headed.gd"
...
func _initialize() -> void:
	print("[UCBV_C4_HEADED] start wave=C4 directive=91 VERIFY_ONLY real_scene=true no_diagnostic_banner=true")
	print("[UCBV_C4_HEADED] evidence=%s" % EVIDENCE_ABS)
	...
	if art != null and art.has_method("set_world_meta_path_override"):
		art.call("set_world_meta_path_override", "user://ucbv_c4_isolated/world_meta.cfg")
...
func _collect_honesty_and_clip_proof() -> void:
	...
	print("[UCBV_C4_HEADED] honesty=%s" % JSON.stringify(_honesty))
...
func _run_state_matrix(tag: String, w: int, h: int) -> void:
	...
	for _i in range(10):
		...
		if not mid.is_empty():
			seen_mods[mid] = true
		print("[UCBV_C4_HEADED] catalog_cycle i=%d mid=%s" % [_i, mid])
	_catalog_proof = {
	...
	if mod_count < 28 and mod_count != 0:
		print("[UCBV_C4_HEADED] WARN catalog_count=%d expected=28" % mod_count)
	if mod_count < 2:
	...
	if not invalid_ok:
		print("[UCBV_C4_HEADED] WARN invalid_feedback soft tag=%s hit_valid=%s can_confirm=%s reason=%s" % [tag, str(hit_valid), str(can_c), validity_reason])
	...
	print("[UCBV_C4_HEADED] viewport %s matrix done captures=%d fails=%d" % [tag, _captures.size(), _failed])
...
func _reset_modes() -> void:
	...
	await _frames(6)
	if bool(_state().get("delete_mode", false)):
		print("[UCBV_C4_HEADED] WARN delete_mode still true after reset")
...
func _teardown_clean() -> void:
	...
	await process_frame
	await process_frame
	print("[UCBV_C4_HEADED] teardown_clean done")
...
func _capture(filename: String, w: int, h: int, state: String, meta: Dictionary) -> void:
	...
	if digest != "" and _sha_seen.has(digest):
		print("[UCBV_C4_HEADED] WARN duplicate_sha %s == %s" % [filename, str(_sha_seen[digest])])
	elif digest != "":
	...
	print("[UCBV_C4_HEADED] captured %s %dx%d sha=%s" % [filename, iw, ih, digest.substr(0, 12)])
...
func _write_runtime_manifest() -> void:
	...
	if f != null:
		...
		f.close()
		print("[UCBV_C4_HEADED] wrote %s" % path)
	var man := {
...
func _finish() -> void:
	...
	print("AIDLE_UCBV001_C4_HEADED=%s checks=%d fails=%d captures=%d inputs=%d" % [
